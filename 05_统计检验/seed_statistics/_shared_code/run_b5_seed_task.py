...
PHM2010 D1/D2/D3, strict TRAIN_SEED isolation.

Unlike B1/B2/B4/B9 (which reuse the run-level tabular feature table +
frozen preprocessing), B5 operates on RAW 7-channel signal -> per-run CWT
scalogram images (force/vibration), so there is no shared frozen-preprocess
directory to verify here. Instead:
  - Stage labels + train/val/test split (vendored_legacy/b5_labels.py) are
    deterministic (no PREPROCESS_SEED dependency -- verified: identical
    logic/thresholds to every other method, no randomness in split
    construction), so no hash-drift risk across seeds within a task.
  - Per-(condition, run_id) CWT images are cached to disk on first build
    (vendored_legacy/b5_dataset.py, B5_IMAGE_CACHE_DIR env var, default
    ../_b5_image_cache/) and reused across ALL tasks/seeds that touch that
    (condition, run_id) -- built once, seed-invariant, same principle as the
    tabular frozen-preprocess artifacts elsewhere in this repo.
  - Raw signal location: PHM2010_ROOT / PHM2010_RAW_ROOT env var (see
    vendored_legacy/b5_signal_preprocessing.py); must point at a directory
    containing c1/c1/c_1_NNN.csv, c1/c1_wear.csv etc. per condition.
  - Evaluation universe: B5's native test coverage is run_id 1-315 (315
    physical runs on the test cutter); reduced to the common run_id 12-315
    (n=304) universe used by every other method before computing metrics
    (shared/phm2010/evaluation_universe.py logic, inlined below).

No probability post-processing (B9-only step); q-MAE/q-RMSE/q-R2/Spearman
are legitimately NaN (this architecture has no continuous degradation-index
head).

Usage:
    python run_b5_seed_task.py --task D1 --train_seed 0 \
        --results_root ../B5_PHM2010_D1_seed_landscape/results \
        --backbone_root ../B5_PHM2010_D1_seed_landscape/backbone_checkpoints
"""
...
def run_one(task: str, train_seed: int, results_root: Path, backbone_root: Path, raw_root: str | None):
    ...
    config_id = {"method": "B5_MultiSource_Attention", "dataset": "PHM2010", "task": task,
    ...
    if raw_root:
        os.environ["PHM2010_RAW_ROOT"] = raw_root
    sys.path.insert(0, str(CODE_DIR))
    import b5_labels as L
    from b5_dataset import ImagePairDataset
    from b5_model import MultiAttentionCNN

    raw_root_used = str(L.PHM2010_RAW_ROOT)
    if not (L.PHM2010_RAW_ROOT / "c1" / "c1_wear.csv").exists():
        raise RuntimeError(
            f"PROTOCOL_FAILED: raw PHM2010 signal data not found at {raw_root_used} "
            f"(expected c1/c1_wear.csv etc.) -- set PHM2010_RAW_ROOT env var or --raw_root"
        )
    print(f"[protocol] PHM2010_RAW_ROOT={raw_root_used}")

    train_df, val_df, test_df = L.get_task_split(train_conditions, test_condition)
    ...
    train_loader = DataLoader(ImagePairDataset(train_df, "label"), batch_size=DEFAULT_CFG["batch_size"], shuffle=True, num_workers=0)
    val_loader = DataLoader(ImagePairDataset(val_df, "label"), batch_size=DEFAULT_CFG["batch_size"], shuffle=False, num_workers=0)
    test_loader = DataLoader(ImagePairDataset(test_df, "label"), batch_size=DEFAULT_CFG["batch_size"], shuffle=False, num_workers=0)
    ...
    full_pred_df = pd.DataFrame({
        "run_id": test_df["run_id"].values.astype(int),
    ...
    # Reduce to the common evaluation universe (run_id 12-315, n=304) shared
    # with every window-based method before computing formal metrics.
    ...
    seed_summary = {
        "Method": "B5_MultiSource_Attention", "Dataset": "PHM2010", "Task": task, "Seed": train_seed,
...
def main():
    ...
    ap.add_argument("--raw_root", default=None, help="Override PHM2010_RAW_ROOT (dir containing c1/c1_wear.csv etc.)")
    args = ap.parse_args()

    out_dir = args.results_root / f"seed{args.train_seed}"
    try:
        status = run_one(args.task, args.train_seed, args.results_root, args.backbone_root, args.raw_root)
