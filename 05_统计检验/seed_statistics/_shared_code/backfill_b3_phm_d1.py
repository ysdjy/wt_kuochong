# -*- coding: utf-8 -*-
r"""
Backfill paired B3 (Multi-task TCN-GRU raw backbone output) results for
already-completed B9_PHM2010_D1_seed_landscape seeds, WITHOUT retraining --
loads the already-saved backbone checkpoint
(results/seed{N}/b9_backbone_seed{N}.pth) and the same frozen preprocessing
used originally, runs only a forward pass (base.predict_model +
base.apply_probability_inference, both deterministic, no gradient step) to
recover B3's raw pre-inference probabilities, exactly as they were at
training time (same checkpoint = same weights = same forward pass output).

Usage:
    python backfill_b3_phm_d1.py
"""
...
D1_DIR = SEED_STATS_DIR / "B9_PHM2010_D1_seed_landscape"
B3_DIR = SEED_STATS_DIR / "B3_PHM2010_D1_seed_landscape"
FROZEN_DIR = EXPAND_ROOT / "shared" / "reproducibility" / "PHM2010_D1_frozen_preprocess"
STAGE_ORDER = ["E", "M", "L"]
...
def import_base():
    os.environ.setdefault("FGDS_RUN_DIR", str(Path(tempfile.gettempdir()) / "b3_backfill_base_side_outputs"))
...
def import_comparison_script(seed_tag: str):
    os.environ["COMPARISON_RECHECK_DIR"] = str(Path(tempfile.gettempdir()) / "b3_backfill_comparison_side_outputs" / seed_tag)
    script_path = CODE_DIR / "7.4对比实验.py"
    mod_name = f"b3_backfill_comparison_{seed_tag}"
...
def main():
    ...
    seed_dirs = sorted((D1_DIR / "results").glob("seed*"), key=lambda p: int(p.name.replace("seed", "")))
    ...
    for sd in seed_dirs:
        ...
        if (b3_out / "DONE.flag").exists():
            n_skipped += 1
            continue
        ckpt_path = sd / f"b9_backbone_seed{seed}.pth"
        ...
        model = base.TCNGRUMultiTask(input_dim, base.BEST_ARCH["channels"], base.BEST_ARCH["gru_hidden"], base.BEST_ARCH["dropout"]) \
            if hasattr(base, "TCNGRUMultiTask") else None
        if model is None:
            raise RuntimeError("Could not locate multitask model class on `base` -- aborting backfill")
        state = torch.load(ckpt_path, map_location="cuda" if torch.cuda.is_available() else "cpu")
        model.load_state_dict(state)
        model = model.to(base.DEVICE if hasattr(base, "DEVICE") else ("cuda" if torch.cuda.is_available() else "cpu"))

        mod = import_comparison_script(f"seed{seed}")
        ...
        row_b3 = mod.metrics_row("B3", "Multi-task TCN-GRU (B9 backbone, raw pre-inference, BACKFILLED) -- D1",
        ...
        b3_summary = {
            "Method": "B3_Multitask_TCN_GRU", "Dataset": "PHM2010", "Task": "D1", "Seed": seed,
        ...
        b3_meta_json = {
            "method": "B3_Multitask_TCN_GRU", "dataset": "PHM2010", "task": "D1", "train_seed": seed,
        ...
        print(f"[backfilled] seed{seed} B3 Acc={b3_summary['Acc']:.4f} MacroF1={b3_summary['Macro-F1']:.4f}")
        n_backfilled += 1

    print(f"\nDone. backfilled={n_backfilled} skipped(already done)={n_skipped} missing_checkpoint={n_missing}")
