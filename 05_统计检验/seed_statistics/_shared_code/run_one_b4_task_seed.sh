#!/bin/bash
# B4 (HTT-Net), PHM2010. Usage: run_one_b4_task_seed.sh "D2,17"
# Set PYTHON_EXE env var to override the interpreter (default: python.exe on PATH).
pair="$1"
task="${pair%%,*}"
seed="${pair##*,}"
SHARED_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
LANDSCAPE_DIR="$SHARED_DIR/../B4_PHM2010_${task}_seed_landscape"
LOG_DIR="$LANDSCAPE_DIR/logs"
mkdir -p "$LOG_DIR"
"${PYTHON_EXE:-python}" "$SHARED_DIR/run_b4_seed_task.py" \
