# -*- coding: utf-8 -*-
r"""
B4 (HTT-Net, Hierarchical Temporal Transformer Network), PHM2010 D1/D2/D3,
strict TRAIN_SEED isolation.

Sibling to run_b1_seed_task.py / run_b2_seed_task.py -- same frozen-preprocessing
source (shared/reproducibility/PHM2010_{task}_frozen_preprocess/), same
base.make_pack(...) windowing (L=12, identical to B1/B2/B3/B9), same output-file
convention, same resume protocol. Only the model + training loop differ: HTTNet
(vendored_legacy/htt_net_model.py, verbatim copy of methods/B4_HTT_Net/code/model.py)
trained with the same optimizer/schedule/early-stop rule as
methods/B4_HTT_Net/adapter.py::HTTNetAdapter (AdamW lr=5e-4, wd=1e-5, epochs=120,
patience=18, grad_clip=1.0, early-stop score = 0.7*(1-val_macro_f1) +
1.0*(1-val_middle_recall) + 0.15*val_loss). No probability post-processing
(B9-only step); q-MAE/q-RMSE/q-R2/Spearman are legitimately NaN (HTT-Net has no
continuous degradation-index head).

Usage:
    python run_b4_seed_task.py --task D1 --train_seed 0 \
        --results_root ../B4_PHM2010_D1_seed_landscape/results \
        --backbone_root ../B4_PHM2010_D1_seed_landscape/backbone_checkpoints
"""
...
HTT_ARCH = {"embed_dim": 32, "depths": (2, 2, 2, 2), "num_heads": 4, "window_size": 3}
...
def import_base():
    os.environ.setdefault("FGDS_RUN_DIR", str(Path(tempfile.gettempdir()) / "b4_task_runner_base_side_outputs"))
...
def train_htt_model(model, tr_pack, va_pack, device: str):
    ...
    for epoch in range(1, TRAIN_CFG["epochs"] + 1):
        ...
        y_true_val = va_pack["ys"]
        p_each, r_each, f1_each, _ = precision_recall_fscore_support(y_true_val, yv, labels=[0, 1, 2], average=None, zero_division=0)
        _, _, macro_f1, _ = precision_recall_fscore_support(y_true_val, yv, labels=[0, 1, 2], average="macro", zero_division=0)
        val_loss = -np.mean(np.log(np.clip(pv[np.arange(len(yv)), y_true_val], 1e-12, 1.0)))
        ...
        epoch_log.append({
            ...
            "val_acc": float(accuracy_score(y_true_val, yv)), "val_macro_f1": float(macro_f1),
...
def run_one(task: str, train_seed: int, results_root: Path, backbone_root: Path):
    ...
    t0 = time.time()
    import torch
    model = HTTNet(input_dim=input_dim, num_classes=3, dropout=base.BEST_ARCH["dropout"], **HTT_ARCH).to(device)
    model, best_info, epoch_log = train_htt_model(model, tr_pack, va_pack, device)
    t1 = time.time()
    ...
    y_pred, prob = predict_stage_model(model, te_pack, device)
    ...
    seed_summary = {
        "Method": "B4_HTT_Net", "Dataset": "PHM2010", "Task": task, "Seed": train_seed,
