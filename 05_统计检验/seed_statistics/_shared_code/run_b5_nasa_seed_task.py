...
NEW adaptation (not a port of an existing baseline, unlike PHM2010's B5) --
see vendored_legacy/b5_nasa_signal_preprocessing.py's module docstring for
the channel-grouping choice (NASA has 6 raw channels -- motor current AC/DC,
2x vibration, 2x acoustic-emission -- grouped into two 3-channel branches by
sensor location, since NASA has no true multi-axis force channel like
PHM2010's Fx/Fy/Fz).

Labels/split come from the same NASA frozen-preprocess artifacts every
other NASA method uses (feat_{train,val,test}_frozen.csv's case/run/
stage_id columns) -- deterministic, no re-derivation. Raw signal itself
comes from mill.mat directly (read-only, via the same in-memory
source-patch import every NASA runner uses).

Evaluation universe: like PHM2010, this raw-signal method's native test
coverage (every run in the test case) is wider than the windowed methods'
(B1/B4/B9's build_sliding_windows can only emit a prediction from run
L onward, 1-indexed) -- restricted to run >= L per test case before scoring,
matching the windowed methods' coverage exactly (verified below, not
assumed).

Usage:
    python run_b5_nasa_seed_task.py --task N1 --train_seed 3 \
        --results_root ../B5_NASA_N1_seed_landscape/results \
        --backbone_root ../B5_NASA_N1_seed_landscape/backbone_checkpoints
"""
...
def run_one(task: str, train_seed: int, results_root: Path, backbone_root: Path):
    ...
    config_id = {"method": "B5_MultiSource_Attention", "dataset": "NASA_Milling", "task": task,
                 "train_cases": hashes["train_cases"], "test_cases": hashes["test_cases"], "train_seed": train_seed}
    ...
    from b5_nasa_dataset import ImagePairDataset
    ...
    full_pred_df = pd.DataFrame({
        "case": feat_test["case"].values.astype(int), "run": feat_test["run"].values.astype(int),
    ...
    # Restrict to run >= L per test case, matching the windowed methods'
    # (B1/B4/B9) coverage -- build_sliding_windows can only emit a
    # prediction from the L-th run of a case onward.
    per_case_max_run_ge_L = full_pred_df.groupby("case")["run"].apply(lambda s: (s >= L).sum())
    pred_df = full_pred_df[full_pred_df["run"] >= L].sort_values(["case", "run"]).reset_index(drop=True)
    if len(pred_df) == 0:
        raise RuntimeError(f"PROTOCOL_FAILED: restricting to run>={L} left 0 test rows (case run-counts: {per_case_max_run_ge_L.to_dict()})")
    ...
    seed_summary = {
        "Method": "B5_MultiSource_Attention", "Dataset": "NASA_Milling", "Task": task, "Seed": train_seed,
