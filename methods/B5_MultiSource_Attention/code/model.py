# -*- coding: utf-8 -*-
"""Multi-source Attention CNN ("Multi-Attention-CNN"), reimplemented from the
old project's `baselines/multi_source_attention/PAPER_SPEC.md` (Wei et al.,
RCIM 2024). Vendored VERBATIM (no architecture/hyperparameter changes) from
`baselines/multi_source_attention/model.py` at
legacy_git_commit 811da096ee47bea4f65db193aa49e793dba6f47d.

Architecture (Table 2):
    Force branch:  CWT image [B,3,224,224]  -> Conv2D(16,k3,s1,same,ReLU)
    Vibration branch: CWT image [B,3,224,224] -> Conv2D(16,k3,s1,same,ReLU)
      -> Concatenate (channel dim)                      -> [B,32,224,224]
      -> Channel-Spatial Attention (Eqs 1-18)            -> [B,32,224,224]
      -> MaxPool(k3,s2,same) -> Conv2D(64) -> MaxPool -> Conv2D(128) -> MaxPool
